...
# wesleyan2parquet.py
# ------------------
# Scraping Wesleyan University student paper opinion pieces with proxy services & 
# saving to S3
# 
# @author Phil Mui
...
LISTING_BASE_URL = f"http://wesleyanargus.com/section/opinion/page/"


DATE_PATTERN = re.compile("http://wesleyanargus.com/(\d+)/(\d+)/(\d+)")
...
def getArticleText(url, numRetries, useProxy=False):
    ...
    while attempts <= numRetries and len(content) < 5: 
        ...
        titleObj = soup.select_one("span > a[href^='#']")
        contentObj  = soup.select_one("div[class^='article-text']")
...
def getArticleList(listUrl, numRetries, showProgress=False, useProxy=False):
    ...
    while attempts <= numRetries and len(articleList) == 0:
        ...
        soup = BeautifulSoup(html, "html.parser")
        articles = soup.select("h2 > a[href^='http://wesleyanargus.com/']")
