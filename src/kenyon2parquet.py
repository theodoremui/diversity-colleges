...
# kenyon2parquet.py
# ------------------
# Scraping Kenyon University student paper opinion pieces with proxy services & 
# saving to S3
# 
# @author Theodore Mui
...
LISTING_BASE_URL = f"https://kenyoncollegian.com/section/opinion/page/"
...
def getArticleText(url, numRetries, useProxy=False):
    ...
    while attempts <= numRetries and len(content) < 5: 
        ...
        titleObj = soup.select_one("h1[class^='o-headline o-headline--title']")
        contentObj  = soup.select_one("div[class^='c-article__body c-article__body--wysiwyg']")
        dateObj = soup.select_one("time[class^='o-timestamp--subtle c-label c-label--strong o-timestamp']")
        if titleObj is not None: content = titleObj.text.strip() + "\n"
        if contentObj is not None:  content += contentObj.text.strip()
        if dateObj is not None: dateObj = datetime.strptime(dateObj.text.strip(), DATE_FORMAT)
        attempts += 1
...
ARTICLE_SELECTOR = \
    "h3[class^='c-story-stream__headline o-headline'] > a[href^='https://kenyoncollegian.com/']"
