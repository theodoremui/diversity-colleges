...
# truman2parquet.py
# ------------------
# Scraping Truman University student paper opinion pieces with proxy services & 
# saving to S3
# 
# @author Theodore Mui
...
LISTING_BASE_URL = f"https://tmn.truman.edu/blog/category/editorial/page/"
...
def getArticleText(url, numRetries, useProxy=False):
    ...
    while attempts <= numRetries and len(content) < 5: 
        ...
        titleObj = soup.select_one("h1[class^='entry-title']")
        contentObj  = soup.select_one("div[class^='entry-content clearfix']")
        dateObj = soup.select_one("span[class^='entry-meta-date updated']")
...
ARTICLE_SELECTOR = \
    "h3[class^='entry-title mh-posts-list-title'] > a[href^='https://tmn.truman.edu/']"
    ...
    for pageNumber in pageList:
        ...
        if len(articleList) == 0: failedPages.append(pageNumber)
            ...
            for article in articleList:
                ...
                if article.text != None and len(article.text) > 0 and url is not None and len(url) > 10 and url not in ['https://tmn.truman.edu/blog/editorial/webcomics/11366/', 'https://tmn.truman.edu/blog/editorial/webcomics/11141/']:
