#########################################################################
# harveymudd2parquet.py
# ------------------
# Scraping Harvey Mudd University student paper opinion pieces with proxy services & 
# saving to S3
# 
# @author Theodore Mui
...
LISTING_BASE_URL = f"https://themuddraker.org/category/opinions/page/"
...
def getArticleText(url, numRetries, useProxy=False):
    ...
    while attempts <= numRetries and len(content) < 5: 
        ...
        contentObj  = soup.select_one("div[class^='content-main'] > div[class^='entry-content']")
        dateObj = soup.select_one("time[class^='entry-date']")
...
ARTICLE_SELECTOR = \
    "h1[class^='entry-title'] > a[href^='https://themuddraker.org/']"
