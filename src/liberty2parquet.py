...
# liberty2parquet.py
# ------------------
# Scraping Liberty University student paper opinion pieces with proxy
...
LISTING_BASE_URL = f"https://www.liberty.edu/champion/category/{SUBJECT}/page/"
...
def getArticleText(url, numRetries, useProxy=False):
    ...
    while attempts <= numRetries and len(content) < 5: 
        ...
        contentObj  = soup.select_one("section[class^='primary'] > article")
        dateObj = soup.select_one("section[class^='primary'] > article time")
