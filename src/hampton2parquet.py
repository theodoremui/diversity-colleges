...
LISTING_BASE_URL = f"https://hamptonscript.com/category/opinion/page/"
...
def getArticleText(url, numRetries, useProxy=False):
    ...
    while attempts <= numRetries and len(content) < 5:
        ...
        titleObj = soup.select_one("h2[class^='entry-title'] > a")
        contentObj  = soup.select_one("main[id^='main'] > div[class^='entry-content']")
        dateObj = soup.select_one("time[class^='entry-date published']")
...
ARTICLE_SELECTOR = \
    "h2[class^='entry-title'] > a[href^='https://hamptonscript.com/']"
