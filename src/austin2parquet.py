...
# austin2parquet.py
# ------------------
# Scraping UT Austin student paper opinion pieces with proxy services & 
# saving to S3
# 
# @author Theodore Mui
...
DATE_PATTERN = re.compile("https://thedailytexan.com/(\d+)/(\d+)/(\d+)")
LISTING_BASE_URL = f"https://thedailytexan.com/category/opinion/page/"
...
def getArticleText(url, numRetries, useProxy=False):
    ...
    while attempts <= numRetries and len(title) < 5 and len(body) < 5: 
        ...
        bodyObj  = soup.select_one("div[role^='main'] > span[class^='storycontent']")
...
ARTICLE_BASE_URL = f"https://thedailytexan.com"
ARTICLE_SELECTOR = \
